...
/* Soft inline error boundary so a render error in the playground NEVER
   bubbles up to the root error page / redirects the user away. */
...
function DeveloperApiInner() {
  ...
  // typing animation (re-runs on lang change OR manual Run press)
  useEffect(() => {
    setTyped("");
    setDone(false);
    setLogLines([]);
  ...
  // auto cycle languages every ~9s after response settles, unless user took control
  useEffect(() => {
  ...
  // live request counter ticking in the status bar
  useEffect(() => {
  ...
  return (
    <section id="developers" className="py-24 md:py-32 bg-mk-bg">
      <div className="max-w-6xl mx-auto px-6">
        <Reveal>
        ...
        <div className="grid lg:grid-cols-[55%_45%] gap-8">
          <Reveal>
            <div
              ...
              onMouseLeave={() => { if (!userTouched.current) setAutoSwitch(true); }}
            >
              {/* ambient glow */}
              <div aria-hidden className="absolute -top-24 -left-24 w-72 h-72 rounded-full bg-mk-orange/20 blur-3xl pointer-events-none" />
              <div aria-hidden className="absolute -bottom-24 -right-24 w-72 h-72 rounded-full bg-blue-500/20 blur-3xl pointer-events-none" />
              {/* scanning line */}
              <motion.div
                aria-hidden
              ...
              {/* window chrome */}
              <div className="relative flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-black/30">
                <span className="w-3 h-3 rounded-full bg-[#ff5f57]" />
                <span className="w-3 h-3 rounded-full bg-[#febc2e]" />
                <span className="w-3 h-3 rounded-full bg-[#28c840]" />
                <div className="ml-3 flex items-center gap-2 text-[11px] text-gray-400 font-mono">
                  <span className="px-2 py-0.5 rounded bg-white/5 border border-white/10">marketon-sdk</span>
                  <span>›</span>
                  <AnimatePresence mode="wait">
                    <motion.span
                      key={lang}
                ...
                <div className="ml-auto flex items-center gap-2">
                  {running ? (
                    <button
                      type="button"
                      onClick={handleStop}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-mono text-rose-300 transition"
                    >
                    ...
                    <button
                      type="button"
                      onClick={handleRun}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-mk-orange/15 hover:bg-mk-orange/25 border border-mk-orange/30 text-[10px] font-mono text-mk-orange transition"
                    >
                ...
                <div className="flex items-center gap-0 px-3 pt-2 overflow-x-auto mk-no-scrollbar">
                  {LANGS.map((l) => (
                    <button
                      ...
                      className={`relative shrink-0 px-3 py-2 text-[11px] font-mono transition ${
              ...
              <div className="relative px-4 pt-4 pb-3">
                <div className="text-[11px] text-gray-500 font-mono mb-2 flex items-center gap-3">
                  <span><span className="text-mk-orange">{ENDPOINTS[lang].split(" ")[0]}</span> {ENDPOINTS[lang].split(" ").slice(1).join(" ")}</span>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="flex items-center gap-1.5">
                      <motion.span
                        ...
                        className={`w-1.5 h-1.5 rounded-full ${running ? "bg-mk-orange" : succeeded ? "bg-emerald-400" : "bg-gray-500"}`}
                      />
                      <span className={running ? "text-mk-orange" : succeeded ? "text-emerald-400" : "text-gray-400"}>
                        {running ? "executing…" : succeeded ? "200 OK" : logLines.length ? "ready" : "idle"}
                      </span>
                    </span>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`flex gap-3 ${
                          isWarn ? "text-amber-300/90" :
                          isOk ? "text-emerald-300/90" :
                          isReq ? "text-sky-300/90" :
                          isTick ? "text-amber-300/90" :
                          "text-gray-400"
                        }`}
                      >
